  ...
  serviceRequests: [
    {
      ...
      referenceNumber: "AIVIS-DEMO-0001",
      status: "UNDER_REVIEW",
      transactionKey: "seniors-card",
...
describe("server app services", () => {
  ...
  it("can return backend shell metadata when backend mode is configured", async () => {
    mockBackendFetch(({ body, init }) => {
      ...
      expect((init?.headers as Headers).get("x-aivis-demo-role")).toBe("TeamLead");
    ...
    vi.stubEnv("AIVIS_FRONTEND_DEMO_ROLE", "TeamLead");
    vi.stubEnv("AIVIS_FRONTEND_DEMO_SUBJECT", "lead@example.test");
  ...
  it("returns mock operations posture only for admin sessions", async () => {
    ...
    vi.stubEnv("AIVIS_FRONTEND_DEMO_ROLE", "Admin");
    vi.stubEnv("AIVIS_FRONTEND_DEMO_SUBJECT", "admin@example.test");
  ...
  it("returns mock reviewer queue, detail and mutation results", async () => {
    vi.stubEnv("AIVIS_FRONTEND_DEMO_ROLE", "ServiceOfficer");
    vi.stubEnv("AIVIS_FRONTEND_DEMO_SUBJECT", "officer@example.test");
  ...
  it("reads backend operations posture through frontend session headers", async () => {
    ...
    mockBackendFetch(({ body, init, url }) => {
      ...
      expect((init?.headers as Headers).get("x-aivis-demo-role")).toBe("Admin");
      expect((init?.headers as Headers).get("x-aivis-demo-subject")).toBe("admin@example.test");
  ...
  it("submits backend drafts and reads generated summary metadata", async () => {
          ...
          data: {
            submitServiceRequest: {
              ...
              serviceRequest: {
                ...
                payload: {},
                referenceNumber: "AIVIS-TEST-0002",
                status: "SUBMITTED",
                transactionKey: "rental-security-subsidy",
                createdAt: "2026-06-12T02:15:00.000Z",
      ...
      return Response.json({
        data: {
          submissionSummary: {
            ...
            fileName: "AIVIS-TEST-0002-summary.txt"
          }
        }
      });
    });
    ...
    await expect(submitTransactionDraft("rental-security-subsidy", backendConfig)).resolves.toMatchObject({
      ...
      summary: {
        filename: "AIVIS-TEST-0002-summary.txt",
        href: "/service-requests/AIVIS-TEST-0002/summary/download"
      }
    });
  });
  ...
  it("reads backend upload policy, uploaded documents and summary downloads", async () => {
    mockBackendFetch(({ body, url }) => {
      if (url.endsWith("/service-requests/AIVIS-TEST-0002/supporting-documents/doc-income/download")) {
      ...
      if (url.endsWith("/service-requests/AIVIS-TEST-0002/summary/download")) {
        return new Response("Reference: AIVIS-TEST-0002", {
          headers: {
            "content-disposition": 'attachment; filename="AIVIS-TEST-0002-summary.txt"',
            "content-type": "text/plain; charset=utf-8"
          }
        });
      }
    ...
    await expect(getUploadedDocuments("rental-security-subsidy", backendConfig)).resolves.toEqual([
      expect.objectContaining({
        category: "Income",
        downloadHref: "/service-requests/AIVIS-TEST-0002/supporting-documents/doc-income/download",
        fileName: "income-evidence.pdf",
        id: "doc-income",
        status: "uploaded"
      }),
    ...
    await expect(getSubmissionSummaryDownload("rental-security-subsidy", "AIVIS-TEST-0002", backendConfig)).resolves.toEqual({
      body: "Reference: AIVIS-TEST-0002",
      contentType: "text/plain",
      filename: "AIVIS-TEST-0002-summary.txt",
      referenceNumber: "AIVIS-TEST-0002"
    });
  ...
  it("maps backend reviewer queue, detail and actions through staff GraphQL headers", async () => {
    mockBackendFetch(({ body, init }) => {
      ...
      expect((init?.headers as Headers).get("x-aivis-demo-role")).toBe("ServiceOfficer");
      ...
      if (body?.query?.includes("FrontendReviewerBatchStatus")) {
        expect(body.variables?.input).toMatchObject({
          referenceNumbers: ["AIVIS-TEST-0002"],
          status: "UNDER_REVIEW"
        });
    ...
    await expect(getReviewerRequestDetailData("AIVIS-TEST-0002", backendConfig)).resolves.toMatchObject({
      activity: [
        {
          description: "Status changed for AIVIS-TEST-0002"
        }
      ],
      payloadItems: [
        {
