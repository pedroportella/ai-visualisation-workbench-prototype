...
export function createMockSessionSummary(env: NodeJS.ProcessEnv = process.env): PrototypeSessionSummary {
  const role = isPrototypeSessionRole(env.AIVIS_FRONTEND_DEMO_ROLE) ? env.AIVIS_FRONTEND_DEMO_ROLE : "Citizen";
  const subject = env.AIVIS_FRONTEND_DEMO_SUBJECT ?? (role === "Citizen" ? mockProfileSummary.email : "officer@example.test");
...
export function createMockSupportingDocumentDownload(
  ...
  return {
    body: [
      "AIVIS prototype supporting document artefact",
