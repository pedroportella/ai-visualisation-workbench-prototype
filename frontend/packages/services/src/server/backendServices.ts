...
function createReviewerHeaders(): HeadersInit {
  return {
    "x-aivis-demo-role": process.env.AIVIS_REVIEWER_DEMO_ROLE ?? "ServiceOfficer",
    "x-aivis-demo-subject": process.env.AIVIS_REVIEWER_DEMO_SUBJECT ?? "officer@example.test"
  };
}

function createFrontendSessionHeaders(): HeadersInit {
  return {
    ...(process.env.AIVIS_FRONTEND_DEMO_ROLE ? { "x-aivis-demo-role": process.env.AIVIS_FRONTEND_DEMO_ROLE } : {}),
    ...(process.env.AIVIS_FRONTEND_DEMO_SUBJECT ? { "x-aivis-demo-subject": process.env.AIVIS_FRONTEND_DEMO_SUBJECT } : {})
  };
}
